...
function registerIpc(): void {
  ...
  ipcMain.handle('pos:checkout', async (event, draft: unknown) => { assertTrustedSender(event); const receipt = database.checkout(draft as any); void cloud.syncNow(); if (getPrinterSettings(database).autoPrint) await printReceipt(database, receipt); return receipt; });
