...
import { getPrinterSettings, listPrinters, printReceipt, printTestReceipt, savePrinterSettings } from './printer';
...
function registerIpc(): void {
  ...
  ipcMain.handle('pos:report-analytics', (event, from, to) => { assertTrustedSender(event); return database.reportAnalytics(String(from), String(to)); });
  ipcMain.handle('pos:activity', (event) => { assertTrustedSender(event); return database.listActivity(); });
  ipcMain.handle('pos:stock-discrepancies', (event) => { assertTrustedSender(event); return database.stockDiscrepancies(); });
  ipcMain.handle('pos:crashes', (event) => { assertTrustedSender(event); return database.crashes(); });
  ipcMain.handle('pos:log-crash', (event, message, source) => { assertTrustedSender(event); database.logCrash(String(message), String(source), app.getVersion()); });
  ipcMain.handle('pos:clear-crashes', (event) => { assertTrustedSender(event); database.clearCrashes(); });
  ...
  ipcMain.handle('pos:remove-payment-method', (event, id: unknown) => { assertTrustedSender(event); const result = database.removePaymentMethod(String(id)); void cloud.syncNow(); return result; });
  ...
  ipcMain.handle('pos:remove-product', (event, id: unknown) => { assertTrustedSender(event); database.removeProduct(String(id)); void cloud.syncNow(); });
  ...
  ipcMain.handle('pos:remove-category', (event, id: unknown) => { assertTrustedSender(event); database.removeCategory(String(id)); void cloud.syncNow(); });
  ipcMain.handle('pos:suppliers', (event, search?: unknown) => { assertTrustedSender(event); return database.listSuppliers(typeof search === 'string' ? search.slice(0, 120) : ''); });
  ...
  ipcMain.handle('pos:remove-supplier', (event, id: unknown) => { assertTrustedSender(event); database.removeSupplier(String(id)); void cloud.syncNow(); });
  ipcMain.handle('pos:supplier-purchases', (event, supplierId: unknown, from?: unknown, to?: unknown) => { assertTrustedSender(event); return database.supplierPurchases(String(supplierId), typeof from === 'string' ? from : undefined, typeof to === 'string' ? to : undefined); });
  ipcMain.handle('pos:supplier-spend', (event, supplierId: unknown, from?: unknown, to?: unknown) => { assertTrustedSender(event); return database.supplierSpend(String(supplierId), typeof from === 'string' ? from : undefined, typeof to === 'string' ? to : undefined); });
  ...
  ipcMain.handle('pos:adjust-stock', (event, productId, quantityDelta, type, reason, details) => { assertTrustedSender(event); const result = database.adjustStock(String(productId), Number(quantityDelta), type as any, typeof reason === 'string' ? reason : undefined, details && typeof details === 'object' ? details as any : undefined); void cloud.syncNow(); return result; });
  ...
  ipcMain.handle('pos:remove-customer', (event, id: unknown) => { assertTrustedSender(event); database.removeCustomer(String(id)); void cloud.syncNow(); });
  ipcMain.handle('pos:customer-ledger', (event, id: unknown) => { assertTrustedSender(event); return database.customerLedger(String(id)); });
  ...
  ipcMain.handle('pos:collect-debt', (event, customerId, amount, methodCode, note) => { assertTrustedSender(event); const receipt = database.collectDebt(String(customerId), Number(amount), String(methodCode), typeof note === 'string' ? note : undefined); void cloud.syncNow(); return receipt; });
  ...
  ipcMain.handle('pos:shop-profile', (event) => { assertTrustedSender(event); return database.shopProfile(); });
  ipcMain.handle('pos:save-shop-profile', (event, profile) => { assertTrustedSender(event); const result = database.saveShopProfile(profile as any); void cloud.syncNow(); return result; });
  ...
  ipcMain.handle('cloud:devices', async (event) => { assertTrustedSender(event); return cloud.devices(); });
  ipcMain.handle('cloud:revoke-device', async (event, id) => { assertTrustedSender(event); await cloud.revokeDevice(String(id)); });
  ipcMain.handle('cloud:billing-status', async (event) => { assertTrustedSender(event); return cloud.billingStatus(); });
  ipcMain.handle('cloud:redeem-code', async (event, code) => { assertTrustedSender(event); return cloud.redeemCode(String(code)); });
  ...
  ipcMain.handle('printer:test', async (event) => { assertTrustedSender(event); return printTestReceipt(database); });
}
