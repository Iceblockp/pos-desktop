...
const labels: Record<PeriodKey,string>={today:'ယနေ့',yesterday:'မနေ့က',thisMonth:'ယခုလ',lastMonth:'ယခင်လ',week:'နောက်ဆုံး ၇ ရက်',rolling30:'နောက်ဆုံး ၃၀ ရက်',custom:'ရက်စွဲသတ်မှတ်မည်'};
...
export function usePeriod(allowAll=false){
  ...
  return {range,label:key==='all'?'ကာလအားလုံး':labels[key],key};
}
export function PeriodFilter({allowAll=false}:{allowAll?:boolean}){
  const value=useSyncExternalStore(subscribe,()=>selection);
  return <div className="flex flex-wrap gap-2 items-center"><select aria-label="ရက်စွဲကာလ" className="select select-bordered select-sm" value={value.key==='all'&&!allowAll?'today':value.key} onChange={e=>update({...value,key:e.target.value as typeof value.key})}>{Object.entries(labels).map(([key,label])=><option key={key} value={key}>{label}</option>)}{allowAll&&<option value="all">ကာလအားလုံး</option>}</select>{value.key==='custom'&&<><input aria-label="စတင်ရက်" className="input input-bordered input-sm" type="date" value={value.from} onChange={e=>update({...value,from:e.target.value})}/><input aria-label="ပြီးဆုံးရက်" className="input input-bordered input-sm" type="date" value={value.to} onChange={e=>update({...value,to:e.target.value})}/></>}</div>;
}
